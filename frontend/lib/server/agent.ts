...
- ACTION_TYPES DE MENSAGEM — só isto é documentado pela Meta: messaging_conversation_started_7d = "Messaging Conversations Started" (conversas iniciadas); messaging_first_reply = "New Messaging Conversations" (conversas novas; a Meta dá só o rótulo, não o cálculo). Os campos messaging_user_depth_N_message_send (2, 3, 5) NÃO têm definição pública e não têm coluna no Gerenciador. Ao citá-los: mostre o valor bruto, diga que a Meta não documenta o significado, e NUNCA explique por atribuição, janela, deduplicação ou "conversa engajada/qualificada" — isso é invenção. Não chame prof. 3 de "conversa qualificada". Se um depth for maior que as conversas iniciadas ou que um depth menor, diga que ele não conta conversas únicas e que não é confiável; não use em relatório de cliente.
